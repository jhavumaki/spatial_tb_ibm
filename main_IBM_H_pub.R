...
for (l in minLHS:maxLHS){
  ...
  klinfo<-klinfo[which(klinfo$lhs==l),]
  ...
  for (curr_t in 1:tspan){
    ...
    if (curr_t>(steady_state+klinfo$j+study_duration)){
      ...
      if (length(TxIDs)>0){
        ...
        screenIDs<-c(unlist(sapply(1:length(unique(inf_hh)), function(x) sampleHH(x, TxIDs, meanHH, housemateDf))))
        hhContacts_states<-state_df[screenIDs,2]
        
        
        
        
        C_states =which(hhContacts_states==2)
        ...
        efficiency[pp, 1]<-length(screenIDs)
  ...
  #save files
  filename_ts= paste('state_ts_state_hhct','lhs', l, 'rng' ,rseed,'seed','HH', klinfo$hh,'_hhct','.csv', sep="")
  write.csv(state_ts,filename_ts)
  
  filename_ts= paste('efficiency_hhct','lhs', l, 'rng',rseed, 'seed','HH', klinfo$hh,'_hhct','.csv', sep="")
  write.csv(efficiency,filename_ts)
  
  filename_ts= paste('output_state_hhct', 'lhs', l, 'rng',rseed,'seed','HH', klinfo$hh,'_hhct','.csv', sep="")
  output<-data.frame(output)
  colnames(output)<- (steady_state+1):tspan
  write.csv(output,filename_ts)
  
  
  filename_ts= paste('out_Inc_put_hhct', 'lhs', l, 'rng',rseed,'seed','HH', klinfo$hh,'_hhct','.csv', sep="")
